...
import { PostPickerModal, PostPickerToolbarButton } from './components/PostPicker';
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const posts = useSelect( ( select ) => {
		...
		return postIds.map( ( id ) => select( 'core' ).getEntityRecord( 'postType', 'post', id ) ).filter( Boolean );
	}, [ postIds ] );
		...
		<>
						...
						<ul style={ { margin: '0 0 12px', padding: 0, listStyle: 'none' } }>
							{ posts.map( ( post ) => (
								...
								>
									...
									<Button
										icon="no-alt"
										label={ __( 'Remove', 'sample-plugin' ) }
										isSmall
										onClick={ () => setAttributes( { postIds: postIds.filter( ( id ) => id !== post.id ) } ) }
									/>
								</li>
							) ) }
						</ul>
			...
			<div { ...useBlockProps( { className: 'dmg-read-more' } ) }>
				{ posts.length > 0 ? (
					...
					<p>{ __( 'Select a post in the block settings.', 'sample-plugin' ) }</p>
				) }
			</div>
		</>
	);
